...
## BOSS-X Pairing Website

This version includes a built-in Express pairing website. Start the bot normally and open the public domain/root URL. The site calls `POST /api/pair`, which uses the same BOSS-X Baileys socket to request a real WhatsApp pairing code.

### Hosting

Use a long-running Node.js host such as Railway, Render, or an Ubuntu VPS/PM2. A static-only host such as GitHub Pages cannot run the pairing backend.

Set `PORT` automatically on hosts that provide it. Do not expose `auth_info` or `bot_data` publicly. Use HTTPS and add rate limiting/authentication before opening the endpoint to unrestricted public traffic.
