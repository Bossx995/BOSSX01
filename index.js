...
// BOSS-X pairing website API
const web = express();
web.disable("x-powered-by");
web.use(express.json({ limit: "10kb" }));
...
web.get("/api/status", (req, res) => {
  res.json({
    ok: true,
    connected: Boolean(pairingSocket?.user),
    account: pairingSocket?.user?.name || pairingSocket?.user?.notify || null
  });
});

web.post("/api/pair", async (req, res) => {
  const number = normalizePairNumber(req.body?.number);
  const now = Date.now();

  if (pairingBusy) return res.status(429).json({ ok:false, message:"A pairing request is already being processed." });
  if (now - lastPairAt < 30000) {
    return res.status(429).json({ ok:false, message:"Please wait 30 seconds before generating another code." });
  }
  if (!/^\d{8,15}$/.test(number)) {
    return res.status(400).json({ ok:false, message:"Enter a valid WhatsApp number with country code." });
  }
  if (pairingSocket?.user) {
    return res.status(409).json({ ok:false, message:"This BOSS-X instance is already connected to a WhatsApp account." });
  }
  if (!pairingSocket || typeof pairingSocket.requestPairingCode !== "function") {
    return res.status(503).json({ ok:false, message:"WhatsApp pairing service is not ready yet. Try again in a few seconds." });
  }
  ...
  try {
    ...
    return res.status(500).json({ ok:false, message:"Could not generate a pairing code. Please try again." });
  } finally {
    pairingBusy = false;
  }
});
